...
image_transport::Subscriber     image_sub_camera;
...
{
	ros::init(argc, argv, "Color_Train");
    ros::NodeHandle n;
    ...
    image_sub_camera = it.subscribe( topic , 1, imageCallback  );

    image_sub_flags = it.subscribe( "image_flags" , 1 , flagsCB);
    image_sub_grass = it.subscribe( "image_grass" , 1 , grassCB);
    image_sub_line  = it.subscribe( "image_line"  , 1 , lineCB);
	image_sub_obst  = it.subscribe( "image_obst"  , 1 , obstCB);
