...
void homographyColorCallback( const sensor_msgs::ImageConstPtr& image_sub_msg)
{
    ...
    box_1.y = robot_center.y - (((params.calibration_height/2) + params.calibration_y) * params.pixels_per_meter);
    
    box_2.x = robot_center.x + (((params.calibration_width/2) + params.calibration_x) * params.pixels_per_meter);
    box_2.y = robot_center.y + (((params.calibration_width/2) - params.calibration_y) * params.pixels_per_meter);
...
void homographyMaskedCallback( const sensor_msgs::ImageConstPtr& image_sub_msg)
{
    ...
    */
    ...
    for(int i = 0; i <= params.laser_res; i++)
    {
        ...
        search_edge.y = (robot_center.y - ((sin(theta) * params.laser_range_max)));
...
int main(int argc, char **argv)
{
    ...
    image_pub_color = it.advertise( "homography_masked" , 1 );
